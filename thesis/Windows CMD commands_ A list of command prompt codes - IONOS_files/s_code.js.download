...
function s_doPlugins(s) {
  ...
  /* server */
  s.server=s.thisHost?s.thisHost:s.server;
  ...
  s.eVar9=s.getQueryParam('ovm_wmid');
  s.eVar17=s.getQueryParam('ias.click');
  s.list1=s.getQueryParam('ias.rules');
  s.eVar70=s.getQueryParam('pfc');
  ...
  if(!s.prop28||s.prop21=='no value')s.prop28=''; //clear max % viewed if no prev page view
 ...
 */
s.getDayNumberSinceLastVisit=new Function(""
  +"var s=this,e=new Date(),cval,ct=e.getTime(),c='s_lastvisit',day=24*"
  +"60*60*1000;e.setTime(ct+3*365*day);cval=s.c_r(c);if(!cval){s.c_w(c,"
  +"ct,e);return 'First page view or cookies not supported';}else{var d"
  +"=ct-cval;days=Math.floor(d/day);days=(days==0)?'Same day':days;s.c_"
  +"w(c,ct,e);return days;}");
/*
...
/* Module: Wywy Integrate  (LEGACY ONLY - this is NOT compatible with AppMeasurement for JavaScript) */
...
var topLevelDomain = hostParts.pop();
var trackingServer = null;

if (topLevelDomain === 'uk') {
  trackingServer = "metrics.ionos.co.uk";
} else {
  trackingServer = "metrics.ionos." + topLevelDomain;
}
